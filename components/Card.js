  ...
  return (
    <a href={`/${news.section}/${news.id}?title=${news.title?.replace(/[\ \/\?\:\;\,\.\|]/g, '-')}`} className={`flex ${right ? 'flex-row':'flex-col'} align-middle justify-center w-full h-full block relative p-2 group`}>
        <div className={`${right ? 'w-1/2 mr-3 mt-2':'bg-gray-800 w-full'}`}>
          <LazyLoadImage onError={e => (e.currentTarget.style.backgroundColor = 'rgb(75 85 99)')} src={news.image_url || img.src} width={300} height={150} alt={news.title || title} className={`border-2 w-full aspect-video opacity-80 ${!right && 'group-hover:opacity-30'} transition-opacity bg-slate-300 text-gray-600 ${hide && 'hidden'} sm:inline-block`} />
        </div>
