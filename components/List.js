    ...
    <div className='border-blue-700 border sm:border-0'>
        ...
        </h3>
        {
            list.map((art, index) => {
                ...
                return <Link onClick={() => popup(url)} href={live ? '#!' : `/${section}/${id}`} key={title + index} className={`flex sm:flex-col sm:justify-center lg:flex-row lg:justify-start p-8 w-full items-center gap-4 ${index % 2 == 0 ? 'bg-gray-100 sm:bg-gray-600 text-gray-400 hover:text-gray-300':'bg-white sm:bg-gray-700 text-gray-400 hover:text-gray-300'}`}>
                    ...
                    <div className=' flex flex-col'>
                        <span className={`${live ? 'text-lg':'text-xl'} font-semibold sm:text-center lg:text-start`}>{title}</span>
