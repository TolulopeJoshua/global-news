    ...
    <div className='border-blue-700 border sm:border-0'>
        ...
        </h3>
        {
            list.map((art, index) => {
                ...
                return <a onClick={() => popup(url)} href={live ? '#!' : `/${section}/${id}`} key={title + index} className={`flex sm:flex-col sm:justify-center lg:flex-row lg:justify-start p-8 w-full items-center gap-4 ${index % 2 == 0 ? 'bg-gray-100 sm:bg-gray-800 text-gray-500 hover:text-gray-400':'bg-white sm:bg-gray-700 text-gray-400 hover:text-gray-300'}`}>
